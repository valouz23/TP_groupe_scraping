...
#Le problème c'est qu'à l'ouverture du site s'affiche une nouvelle frame pour demander notre consentement à l'utilisation de données
#Et il faut clique sur "accepter et fermer" ou "continuer sans accepter" pour accéder au site. Le truc c'est que pour ça ils utilisent
#un iframe, donc il faut changer de frame pour accéder au bouton, et ensuite revenir au frame de départ d'où le code suivant
#(aussi je laisse un peu de temps avant de revenir à la frame de départ sinon il a pas le temps de garder l'info comme quoi j'ai cliqué)
...
#voici une fonction qui permet de se rendre sur la bibliographie d'un auteur en prenant en argument son nom, et qui rend le nombre de
#page de cette bibliographie (ce sera utile poour plus tard, parce que la pagination n'est pas liée à l'url pour ce site)
...
liste_liens=[]
for auteur in writers_list:
    n=go_to_list_of_books(auteur) #on récupère le nombre de page, et au passage on va sur la bibliographie
    for i in range(int(n)-1): #pour chaque page de la bibliographie
        ...
        for url in liens:
            liste_liens.append(url.get_attribute("href")) #on récupère le lien dans l'attribut et on l'ajoute à la liste de liens
...
with open("donnees_livres.csv", "w", newline='') as f:
    writer =csv.writer(f)
    writer.writerow(['Titre', 'Auteur', 'Note/5', 'Résumé'])
